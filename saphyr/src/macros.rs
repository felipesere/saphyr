...
impl $(< $( $generic ),+ >)? crate::index::Accessor<$yaml> for usize $(where $($whereclause)+)? {
    fn index_into<'a>(self, yaml: &'a $yaml) -> Option<&'a $yaml> {
        yaml$(.$accessor)?.as_sequence_get(self)
    }
}

#[allow(clippy::needless_lifetimes)]
impl $(< $( $generic ),+ >)? crate::index::Accessor<$yaml> for &str $(where $($whereclause)+)? {
    fn index_into<'a>(self, yaml: &'a $yaml) -> Option<&'a $yaml> {
        yaml$(.$accessor)?.as_mapping_get(self)
    }
}

#[allow(clippy::needless_lifetimes)]
impl $(< $( $generic ),+ >)? crate::index::Accessor<$yaml> for String $(where $($whereclause)+)? {
    fn index_into<'a>(self, yaml: &'a $yaml) -> Option<&'a $yaml> {
...
// I don't think these are strictly needed?
//
// define_yaml_object_safely_index!(
